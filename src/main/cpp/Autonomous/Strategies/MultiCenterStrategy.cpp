...
void MultiCenterStrategy::Init(std::shared_ptr<World> world) {
    ...
    const double pushBackSpeed = PrefUtil::getSet("Auto.MCS.PushBack.y", -0.15);


    // Drive off platform
    const double initialDriveSpeedY = PrefUtil::getSet("Auto.MCS.initDriveSpeed.y", 0.3);
    const double initialDriveSpeedX = PrefUtil::getSet("Auto.MCS.initDriveSpeed.x", 0.0) * inv;
    ...
    const double cargoDrive1Y = PrefUtil::getSet("Auto.MCS.cargoDrive1.y", 0.4);
    const double cargoDrive1X = PrefUtil::getSet("Auto.MCS.cargoDrive1.x", 0.1) * inv;
    const double cargoDrive1Time = PrefUtil::getSet("Auto.MCS.cargoDrive1.time", 1.5);
    ...
    const double cargoDrive2Y = PrefUtil::getSet("Auto.MCS.cargoDrive2.y", 0.3);
    const double cargoDrive2Time = PrefUtil::getSet("Auto.MCS.cargoDrive2.time", 5.0);
    const double cargoDrive2IgnoreTime = PrefUtil::getSet("Auto.MCS.cargoDrive2.ignoretime", 0.0);
    const int cargoDrive2Target = PrefUtil::getSet("Auto.MCS.cargoDrive2.tgt", 3);
    ...
    const double pickupFastSpeed = PrefUtil::getSet("Auto.MCS.pickupFastSpeed.y", -0.4);
    const double pickupFastSpeedX = PrefUtil::getSet("Auto.MCS.pickupFastSpeed.x", 0.2) * inv;
    const double pickupFastSpeedRamp = PrefUtil::getSet("Auto.MCS.pickupFastSpeedRamp", 0.5);
    const double pickupFastTime1 = PrefUtil::getSet("Auto.MCS.pickupFastTime1", 3.0);
    steps.push_back(new ConcurrentStep({
    ...
    // Pickup hatch
    const double pickupVizSpeed = PrefUtil::getSet("Auto.MCS.pickupVizSpeed", 0.25);
    const double pickupVizTime = PrefUtil::getSet("Auto.MCS.pickupVizTime", 5.0);
    const double pickupVizTimeout = PrefUtil::getSet("Auto.MCS.pickupVizTimeout", 2.5);
    ...
    // Drive to cargoship
    const double cargoDrive3Y = PrefUtil::getSet("Auto.MCS.cargoDrive3.y", 0.4);
    const double cargoDrive3X = PrefUtil::getSet("Auto.MCS.cargoDrive3.x", -0.3) * inv;
    const double cargoDrive3Time = PrefUtil::getSet("Auto.MCS.cargoDrive3.time", 1.5);
    ...
    // Drive and Stop at Target
    const double cargoDrive4Y = PrefUtil::getSet("Auto.MCS.cargoDrive4.y", 0.3);
    const double cargoDrive4Time = PrefUtil::getSet("Auto.MCS.cargoDrive4.time", 5.0);
    const double cargoDrive4IgnoreTime = PrefUtil::getSet("Auto.MCS.cargoDrive4.ignoretime", 0.0);
    const int cargoDrive4Target = PrefUtil::getSet("Auto.MCS.cargoDrive4.tgt", 2);
...
void MultiCenterStrategy::DriveAndPlaceHatch(double cargoShipAngle, int inv) {
    const double placeHatchY = PrefUtil::getSet("Auto.MCS.placeHatchY", 0.3);
	const double placeHatchThreshold = PrefUtil::getSet("Auto.MCS.placeHatchThreshold", 5.0);
	const double placeHatchTimeout = PrefUtil::getSet("Auto.MCS.cargoDriveTimeout", 3.5);
    const double pushBackSpeed = PrefUtil::getSet("Auto.MCS.PushBack.y", -0.15);
