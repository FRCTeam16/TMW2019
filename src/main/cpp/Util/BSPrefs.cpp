...
BSPrefs::BSPrefs() {
    ...
    lookupDouble.emplace("FLOff", 170.156);
    lookupDouble.emplace("FROff", 38.3203);
    ...
    lookupDouble.emplace("RLOff", -127.969);
    lookupDouble.emplace("RROff", -34.3652);
...
double BSPrefs::GetDouble(std::string key) const {
...
int BSPrefs::GetInt(std::string key) const {
...
bool BSPrefs::GetBool(std::string key) const {
...
bool BSPrefs::GetBool(std::string key, bool value) const {
    if (lookupBool.count(key) > 0) {
        ...
        std::cout << "!!! BSPrefs::getBool Missing Key: " << key << " | " << value << "\n";
        return value;
    }
}